...
p.value.age = 1-pt(t, df=n.deaths-1)

round(p.value.age, digits=4)
```

> The test statistic is:
>
> $t=\frac{`r round(af.mean.age, 2)`-`r round(pop.mean.age)`}{\frac{`r round(sd.age, 2)`}{\sqrt{`r n.deaths`}}}=`r round(t, 2)`$
...
p.state.comp <- (pnorm(z.state.comp))*2
